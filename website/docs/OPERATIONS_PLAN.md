...
- **更新方法**：
  ...
  2. Lark #web-ops チャンネルで更新完了を共有。
- **リスク**：未更新だと「準備中」が表示されたままになる → リマインダを作成（Google Calendar or Lark Workflow）して更新漏れを防止。
...
- **Larkチャンネル案**：`#web-ops`（更新連絡・レビュー依頼）、`#dojolive`（無料稽古運用）、`#fellow-ops`（Fellow応募対応）。
- **週次タスク例**：
...
2. 各チームに更新担当者とSLAを共有、Google CalendarやLarkリマインダで自動化。
